...
export default function GroupsSidebar() {
  ...
  return (
    <aside className="boder-r flex h-full w-80 flex-col border-r bg-neutral-100 shadow-md dark:border-r-neutral-800 dark:bg-zinc-900">
      ...
      <div className="sticky top-0 z-10 flex flex-col gap-3 border-b border-zinc-200 bg-neutral-100 px-6 pb-4 pt-6 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        {/* Sticky Nav */}
        <div className="sticky top-[120px] z-10 mb-2 mt-4 rounded-lg bg-white shadow-sm dark:bg-zinc-800 ">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 rounded-lg px-4 py-2 text-base font-medium transition-colors ${isActive ? 'bg-zinc-800 text-zinc-50 shadow dark:bg-zinc-100 dark:text-zinc-800' : 'text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800 dark:hover:text-white'}`}
      ...
      {/* Scrollable Content */}
      <div className="scrollbar-thin scrollbar-thumb-zinc-400 dark:scrollbar-thumb-zinc-800 scrollbar-track-transparent hover:scrollbar-thumb-zinc-500 dark:hover:scrollbar-thumb-zinc-600 flex-1 overflow-y-auto px-6 pb-6 pt-4">
        {/* Groups You Manage Section */}
        <div>
          <div className="mb-2 px-1 text-sm font-semibold uppercase tracking-wide text-zinc-800 dark:text-zinc-50">
            Groups You Manage
          </div>
          <ul className="space-y-1 pr-1">
            ...
            {filteredManagedGroups.map((group) => (
              <li key={group.id}>
                <button
                  onClick={() => router.push(`/groups/${group.id}?from=manage`)}
