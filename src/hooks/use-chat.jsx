...
export const useChat = (user, chatPartner) => {
      ...
      updateMessages(prev => {
        ...
        if (existingIndex !== -1) {
          ...
          updated[existingIndex] = {
            ...updated[existingIndex],
            ...newMessage,
            id: newMessage.id || updated[existingIndex].id,
            messageState: 'sent',
  ...
  const sendMessage = async (content, files, receiverId) => {
    ...
    // ✅ BULLETPROOF: Prevent duplicate sends with stronger key
    const sendKey = `${user.id}-${target}-${content}-${currentTime}`;
    ...
    // ✅ SIMPLIFIED: Create optimistic message
          ...
          const updated = prev.map(msg => 
            msg.id === messageId 
              ? {
                  ...msg,
                  ...backendResponse,
                  id: backendResponse?.id || messageId,
                  messageState: 'sent',
                  isOptimistic: false,
                  backendConfirmed: true,
                  isFailed: false,
